...
# USAGE: python years.py <NAME> <AGE AS AN INTEGER>
...
name = sys.argv[1] # <== To access the values in an array, you use the [] operator. For instance, to access the first item in the command line, you would access sys.argv[0]. Which would return the name of the script being run. This would be associated with the <NAME> argument above.
age = int(sys.argv[2]) # <== The arguments passed to the script are stored in the array starting with index 1. Let's make our script take the 2 arguments name and age which will be indexes 1 and 2 respectively. We can assign these values to a named variable and then print them. This would be associated with the <AGE AS AN INTEGER> value above.
...
## NOTES: Any values that do not correlate with the variable type, will fail out the script. For example, the $name variable is expecting a literal value. The #age variable is expecting an integer. Try swapping them around to see what happens.
